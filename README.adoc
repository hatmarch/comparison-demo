...
[NOTE]
====
You can let the operator create the password for you.  If you follow this route, you'll need to fish out the password yourself.  Assuming the cluster is named `petclinic-mysql` (running this command in the project in which it is installed):

----
oc get secret petclinic-mysql-root-password -o jsonpath="{.data.password}" | base64 --decode
----
====
