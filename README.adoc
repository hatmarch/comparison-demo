...
[TODO]
====
0. Try to figure out why resources are not being rebuilt (changing the image does not change what's packed it appears)
1. Setup another environment (prod) checking the setup setups
2. Setup a pipeline to push to other environment (can steal from link:https://github.com/siamaksade/tekton-cd-demo[this tutorial])
...
2. Attempt to use a workspace to further speed up the build (doesn't appear to have made a significant difference to build times in the way that it's currently used in the tutorial)
----
[INFO] BUILD SUCCESS
[INFO] ------------------------------------------------------------------------
